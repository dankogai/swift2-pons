...
for i in 0...4 {
    ...
    print("exp(+\(i)/2) ==",  ep)
    print("exp(-\(i)/2) ==",  em)
    let lp = Rational.log(ep, precision:64)
    print("log(+(i)/2) ==", qp)
    let lm = Rational.log(ep, precision:64)
    print("log(exp(\(i)/2)) - (\(i)/2) ~=",  (qp - lm).toDouble())
}
print("exp(sqrt(2))==", Rational.exp(Rational.sqrt(+BigInt(2).over(1)), precision:64))
